...
| the numbers | `tools/pm/board.py`, via the `bead-report` skill and the `bead-forecast` skill |
...
```bash
python3 tools/pm/board.py report --json > /dev/null && python3 tools/pm/board.py report
```
...
Inside a dynamic `/loop`, stopping means calling `ScheduleWakeup` with `stop: true`.
