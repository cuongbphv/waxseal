...
### G6 — the on-chain layer shipped with two known, deliberate deviations [Shipped, with gaps stated]
...
flagged by the agents who found them rather than silently absorbed, and both are still open as
`needs-human` beads at the time of this entry — this ledger's discipline is to describe what actually
...
- **waxseal-fg4.45 — the ledger dimension does not raise τ.** The plan's Workstream F4 prose describes
  ...
  genuinely checking something real; the gap is in the REPORTED COUNT, not in the underlying check.
...
[`tests/test_cli_ledger_e2e_anvil.py`](../../tests/test_cli_ledger_e2e_anvil.py)). Both gaps are about
