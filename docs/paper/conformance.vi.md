...
### G6 — tầng on-chain ship kèm hai khác biệt đã biết, cố ý [Shipped, kèm khoảng trống]
...
ghi nhận công khai thay vì âm thầm nuốt, và cả hai vẫn còn mở dạng bead `needs-human` tại thời
điểm ghi mục này — kỷ luật của sổ này là mô tả cái ĐÃ SHIP, không phải mô tả nguyên văn chưa sửa
...
- **waxseal-fg4.44 — `registry_absent` và `registry_unreachable` là MỘT status, không phải hai.**
  ...
  mà chính dòng ở trên vừa khen là exhaustive hay không.
- **waxseal-fg4.45 — dimension ledger không nâng τ.** Văn bản Workstream F4 của plan mô tả nối
  ...
  trống nằm ở CON SỐ ĐƯỢC BÁO CÁO, không nằm ở bản thân check.
...
trống nằm ở RÌA của bề mặt đã ship — một state từ vựng bị gộp, một đóng góp vào τ chưa được đếm —
