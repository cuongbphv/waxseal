...
class TestAppend:
    def test_event_is_appended_to_the_trail(self, handler, tmp_path: Path) -> None:
    ...
    def test_multiple_events_form_one_verified_chain(self, handler, tmp_path: Path) -> None:
...
class TestRedaction:
    def test_secret_in_message_never_reaches_disk(self, handler, tmp_path: Path) -> None:
...
class TestNeverBlocksThePipeline:
    ...
    def test_none_context_is_fine(self, handler, tmp_path: Path) -> None:
